...
6. Access a place for global announcements, forums, and other necessities not yet accomplished with existing Meower builds.
7. Access hosting for public communities
...
Some responsibilities of user servers include:
1. Storing content shared within that server.
2. Serving as a substitute in the event that the main server is down.
3. Functioning similarly to a CDN. Servers will talk to each other (utilizing the main server as a "phonebook") and share posts, videos, music, etc. to improve load times for clients around the world.
4. Offering hosting for public/private communities

## Client and user server communication

Users can utilize user servers to:
1. Keep main server load down
